...
export default function StarDate() {
  ...
  const [zodiac, setZodiac] = useState<{name: string, emoji: string} | null>(null);
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400">
        <div className="min-h-screen backdrop-blur-sm bg-white/10">
          ...
          <div className="text-center mb-8 animate-fade-in">
            ...
            <p className="text-xl text-white/90 font-light">Discover your celebrity birthday twins</p>
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-8 bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-xl">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-700 font-semibold">{celebritiesDate.length} Celebrities in Database</span>
              </div>
              <button
                onClick={() => {
            ...
            <div className="max-w-6xl mx-auto mb-12 animate-fade-in">
              <div className="bg-white/95 backdrop-blur-md rounded-3xl shadow-2xl p-8">
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <h2 className="text-3xl font-bold text-gray-800">Browse All Celebrities</h2>
                </div>
                ...
                <div className="mb-4">
                  <p className="text-gray-600 font-medium">
                    Showing {filteredCelebrities.length} of {celebritiesDate.length} celebrities
                  </p>
                </div>
                      ...
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/50 to-transparent p-3">
                          <div className="flex items-center gap-2 text-yellow-300">
                            ...
                            <span className="text-xs font-semibold">{celeb.achievement}</span>
                          </div>
                        </div>
                      </div>
                      <div className="p-5">
                        <h4 className="text-xl font-bold text-gray-800 mb-2">{celeb.name}</h4>
                        <p className="text-sm text-purple-600 font-semibold mb-3 flex items-center gap-1">
                          <Calendar className="w-4 h-4" />
                          {monthNames[celeb.month - 1]} {celeb.day}
              </p>
                        <p className="text-gray-600 text-sm leading-relaxed line-clamp-3">{celeb.bio}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {filteredCelebrities.length === 0 && (
                  <div className="text-center py-12">
                    <p className="text-xl text-gray-500">No celebrities found matching your criteria</p>
                    <button
                      onClick={clearFilters}
              ...
              <div className="max-w-2xl mx-auto mb-8 bg-white/90 backdrop-blur-md rounded-2xl p-6 shadow-xl animate-fade-in">
                <div className="text-center">
                  <p className="text-3xl mb-2">{zodiac.emoji}</p>
                    <p className="text-lg font-semibold text-gray-800">Your Zodiac Sign: <span className="text-purple-600">{zodiac.name}</span></p>
                </div>
              </div>
            )}
                ...
                {matches.length > 0 ? (
                  <>
                    <h3 className="text-3xl font-bold text-white text-center mb-8 drop-shadow-lg">
                        Found {matches.length} Birthday Twin{matches.length > 1 ? 's' : ''}! 🎉
                    </h3>
                        ...
                        >
                          <div className="relative h-64 overflow-hidden">
                            ...
                            <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/50 to-transparent p-4">
                              <div className="flex items-center gap-2 text-yellow-300">
                                <Star className="w-4 h-4 fill-current" />
                                <span className="text-sm font-semibold">{celeb.achievement}</span>
                              </div>
                            </div>
                          ...
                          <div className="p-6">
                            <p className="text-gray-600 leading-relaxed mb-4">{celeb.bio}</p>
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-purple-600 font-semibold">
                                {getZodiacSign(celeb.month, celeb.day).emoji} {getZodiacSign(celeb.month, celeb.day).name}
                              </span>
                  ...
                  <div className="text-center bg-white/90 backdrop-blur-md rounded-3xl shadow-2xl p-12 animate-fade-in">
                    ...
                    <h3 className="text-2xl font-bold text-gray-800 mb-4">No Birthday Twins Found</h3>
                    <p className="text-gray-600 mb-6">
                      No celebrities share your birthday in our database, but you're one of a kind! ✨
                    </p>
                    <button
